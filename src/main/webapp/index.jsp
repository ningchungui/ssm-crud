...
<script type="text/javascript">
	...
	//1、页面加载完成以后，直接去发送ajax请求,要到分页数据，注意：前面有一个“$”符号
	/* $(function() {
		//去首页
		to_page(1);
 ...
 */
	...
	function build_emps_table(result) {
		//清空table表格，"#emps_table tbody"指emps_table下的tbody
	...
	function reset_form(ele) {
		$(ele)[0].reset();//后面的[0]表示使用js的方法，应为jQuery没有reset()方法
		//清空表单样式，find("*")指后面的所有元素
	...
	$(document).on("click", ".edit_btn", function() {
		...
		//1、查出部门信息，并显示部门列表
		getDepts("#empUpdateModal select");//empUpdateModal的下拉列表
		//2、查出员工信息，显示员工信息
		//当前按钮的edit-id属性值
	...
	$("#emp_update_btn").click(function() {
		...
		$.ajax({
			...
			type : "PUT",
			data : $("#empUpdateModal form").serialize(),//调用serialize方法将表单数据序列化为bean对象
			success : function(result) {
				//alert(result.msg);
				//1、关闭对话框
